...
*/
phonecatControllers.controller('PhoneDetailCtrl', ['$scope', '$routeParams', 'Phone', 'socket',
  function($scope, $routeParams, Phone, socket) {
    $scope.phones = Phone.query({}, function() {
	angular.forEach($scope.phones, function(phone){
	   if(phone.id == $routeParams.phoneId) {$scope.name = phone.name; $scope.desc = phone.desc; }
	});
    });
	...
	var oldPhones = $scope.phones;
	var newPhones = [];
	console.log($scope.code+$('#name').val() +$('#desc').val());
	oldPhones.forEach(function(phone, index, array) {
	   if(phone.id == $scope.code ) {
		phone.name=$('#name').val();
		phone.desc=$('#desc').val();
	   }
